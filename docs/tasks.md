...
- [ ] **NEW** `src/core/policy.ts` — central `POLICY_DATA` object
...
- [ ] **MODIFY** `src/scenes/GameScene.ts` — remove:
  - `handlePointerMove`
  - `handlePointerDown`
  - `reconstructCityFromState`
  - `spawnAmbientCommuter`
  - `updateBuildingStatuses`
- [ ] Checkpoint: game boots, empty/static map, HUD still renders

---

## Phase 3 — Core sim math (the "does it feel like Democracy" phase)

- [ ] **MODIFY** `src/systems/EconomyManager.ts`:
...
- [ ] **MODIFY** `src/systems/PolicyManager.ts`:
  ...
  - Read effect values from `POLICY_DATA` (`policy.ts`) instead of
    inline numbers
- [ ] **NEW** population-0 game-over check (wherever turn resolution
...
- [ ] **NEW** development-threshold table — defines what population/
...
- [ ] **NEW** visual-render step — either `src/systems/CityVisualManager.ts`
      or a method folded into `GameScene.ts`; takes population/pollution/
      approval from `GameState`, scatters building sprites procedurally
- [ ] **MODIFY** `src/core/Constants.ts` — `BuildingTypeConfig` shrinks to
...
- [ ] **DELETE** `src/systems/GridManager.ts` — entire file
- [ ] **DELETE** `src/objects/Building.ts` — entire file
- [ ] **DELETE** `src/objects/Citizen.ts` — entire file
- [ ] **MODIFY** `src/systems/SaveManager.ts` — drop `gridData` field
      from `SavePayload`
...
- [ ] **NEW** persistent policy side panel (replaces `createPolicyModal`
      as an always-visible panel) — tax rate slider + 3 toggles
- [ ] **NEW** grid layout/tilt — shrink grid, center it, isometric/tilted
...
- [ ] Run `npx tsc --noEmit` — repeat after each phase above, not just
      at the end, to catch type errors from deletions early
- [ ] Manual check: policies adjustable in side panel → turn advances →
